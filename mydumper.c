...
static GOptionEntry entries[] =
{
	...
	{ "max-retained-dumps", 'n', 0, G_OPTION_ARG_INT, &max_retained_dumps, "Maximum number of backups to be retained", NULL},
...
int main(int argc, char *argv[])
{
	...
	if (daemon_mode) {
		...
		for (guint i = 0; i < max_retained_dumps; i++)
		{
			char *dump_directory = g_strdup_printf("%s/%d", output_directory, i);
...
void *exec_thread(void *data) {
	...
	while(1) {
		...
		if (!shutdown_triggered) {
			...
			char dump_symlink_source[16];

			sprintf(dump_symlink_source, "%d", dump_number);

			dump_number++;
			dump_number %= max_retained_dumps;
